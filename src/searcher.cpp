...
boost::shared_ptr<Graph::Path_type> Searcher::find_shortest_path(
...
boost::shared_ptr<Graph::Path_type> Searcher::find_breadth_first(
        ...
        const Graph::Node_type& goal, unsigned int depth_limit) const
{
    ...
    if (agenda.size() == 0)
        return boost::shared_ptr<Graph::Path_type>();
    ...
    if (current_node == goal)
        return boost::shared_ptr<Graph::Path_type>(
                new Graph::Path_type(current_path));
